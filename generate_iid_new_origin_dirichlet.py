...
def get_dataset(mode='train'):
    ...
    #dataset = CIFAR10(root='./data', train=True if mode=='train' else False, download=True, transform=transform_train)

    #dataset = CIFAR10(root='./data', train=True if mode=='train' else False, download=True, transform= transform_train if mode=='train' else transform_test )

    dataset = CIFAR10Instance(root='./data', train=True, download=True,
                              transform=None)

    dataset._split_iid_train_data(5)
...
def devide_train_data(data, n_sample, SRC_CLASSES, NUM_USERS, min_sample, alpha=0.5, sampling_ratio=0.5):
    ...
    while min_size < min_sample:
        ...
        for l in SRC_CLASSES:
            ...
            #proportions = [ 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500]
...
def divide_test_data(NUM_USERS, SRC_CLASSES, test_data, Labels, unknown_test):
    ...
    for user in trange(NUM_USERS):
        if user != 0 : continue ###只执行1个client
...
def main():
    ...
    parser.add_argument("--min_sample", type=int, default=200, help="Min number of samples per user.")
    parser.add_argument("--sampling_ratio", type=float, default=1.0, help="Ratio for sampling training samples.")
    parser.add_argument("--unknown_test", type=int, default=0, help="Whether allow test label unseen for each user.")
    parser.add_argument("--alpha", type=float, default=100.0, help="alpha in Dirichelt distribution (smaller means larger heterogeneity)")
